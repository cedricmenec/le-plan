...
- [~] Task: Update `MissionHeaderHero.tsx` title styling and metadata layout
    - [ ] Change title font size from `text-5xl` to `text-3xl`
    - [ ] Move Project Name, Type, and Status to a single row above the title
    - [ ] Remove `shadow-md` and other shadow classes from the hero container
- [ ] Task: Remove redundant metadata grid from Mission Detail page
    - [ ] Locate and remove the grid containing Estimation, Confidence, and Delivery Dates in `app/missions/[id]/page.tsx`
