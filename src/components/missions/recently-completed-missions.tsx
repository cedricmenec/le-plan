...
import { calculateMissionDurations } from '@/lib/missions/duration-utils'
...
type StatusHistoryEntryLite = { status: string; reason: string | null; created_at: string };
...
type MissionWithTasks = {
  ...
  status_history?: StatusHistoryEntryLite[]
}
            ...
            missions.map((mission) => {
              ...
              const durations = calculateMissionDurations(
                (mission.status_history || []).map(h => ({
                  status: h.status as MissionState,
                  reason: h.reason as MissionReason | null | undefined,
