...
## Phase 3: Drag & Drop Reordering [checkpoint: 2cecae4]

- [x] Task: Integrate `@dnd-kit` in `TaskList`
    - [x] Wrap task items with `DndContext`, `SortableContext`, and `VerticalSortStrategy`.
    - [x] Implement `SortableItem` wrapper for individual tasks.
    - [x] Add drag handle or allow dragging the whole item (to be determined during implementation for best UX).
- [x] Task: Server Action for Reordering
    - [x] Write tests for the reordering logic and server action.
    - [x] Create server action to update multiple task positions in a single transaction (if possible) or efficiently.
    - [x] Implement optimistic UI for reordering with "pending" visual feedback.
- [x] Task: Conductor - User Manual Verification 'Phase 3: Drag & Drop Reordering' (Protocol in workflow.md) (2cecae4)
