...
export function calculateMissionDurations(
  ...
  const normalized = sortedHistory.map(h => ({ ...h, created_at: toDate(h.created_at) }));
  const firstActiveIndex = normalized.findIndex(h => h.status === MissionState.Active);
  const firstActiveDate = firstActiveIndex !== -1 ? normalized[firstActiveIndex].created_at : null;

  const terminatedIndex = normalized.findIndex(h => h.status === MissionState.Terminated);
  const endDate = terminatedIndex !== -1 ? normalized[terminatedIndex].created_at : now;
